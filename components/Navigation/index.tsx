...
import { INavigationResponse, useGetNavigationsQuery } from '../../features/navigations/index';
import {
    useAppDispatch,
    useAppSelector,
  } from '../../app/hooks';
import { isOpenMenu, closeMenu } from '../../features/menu';
import Icon, { FacebookFilled, HomeFilled, HomeOutlined, InstagramFilled, PhoneFilled, PhoneOutlined } from '@ant-design/icons';
...
export const Navigation = () => {
    ...
    const [activeCategoryId, setActiveCategoryId] = useState<number>(data.length > 0 ? data[0].id : 0)
    const [categoryItems, setCategoryItems] = useState<INavigationResponse[]>([]);

    useEffect(() => {
        if(data.length > 0) {
            const resultActiveCategory: any = data.find((element) => (element.id === activeCategoryId))
    ...
    return (
        <>
            <Drawer
                ...
                >
                    <div className='social-links'>
                        ...
                        <Link href={"/telegram"}>
                            <div className='social-link'>
                                <Icon component={() => (<img style={{width: 24, transform: "translateY(-6px)"}} src="/icons/telegram-48.png"/>)} />
                            </div>
                        </Link>
                    </div>
                    <div className='const-links'>
                        <Link href={"/contact_us"}>
                            <div className='const-link'>
                                Contact us
                            </div>
                        </Link>
                        <Link href={"/about_us"}>
                        ...
                        <Col xs={24} sm={24} md={5} className='categories'>
                            {
                                data.map(item => {
                                    return (
                                        <div onClick={() => handleClickCategory(item.id)} className={activeCategoryId === item.id? 'category category-active' : 'category'} key={item.id} > {item.title}</div>
                                    )
                                })
                            }
                        </Col>
                                                ...
                                                <div style={{paddingLeft: 10, height: "100%" }}>
                                                    <h2 style={{color: "#eaa83c"}}>{item.title}</h2>
                                                    {
                                                        item.children.map(sub_item => {
                                                            return <div className='category-item' key={sub_item.id}><Button type="link" href={sub_item.path}>{sub_item.title}</Button></div>
                                                        })
                                                    }
                                                </div>
                                            </Col>
            ...
            <Drawer
                ...
                placement="left"
                closable={false}
                style={{ position: 'absolute', backgroundColor: "#333333"}}
            >
                ...
                <Row gutter={[5,40]}>
                    {
                        categoryItems?.map(item => {
                            return <Col span={24} key={item.id} >
                                <div style={{paddingLeft: 10, height: "100%" }}>
                                    <h2 className='subcategory-header' style={{color: "#eaa83c"}}>{item.title}</h2>
                                    {
                                        item.children.map(sub_item => {
                                            return <div className='category-item' key={sub_item.id}>
            ...
            <style jsx>{`
                ...
                .category-active, .category.category-active:hover {
